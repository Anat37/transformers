...
class ConvbertLSTMForSequenceClassification(ConvbertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        ...
        self.lstm = nn.LSTM(config.hidden_size, config.hidden_size, bidirectional=True, batch_first=True)
    ...
    def forward(
        ...
        #pooled_output = self.dropout(pooled_output)
        _, (h, _) = self.lstm(outputs[0])
        logits = self.classifier(torch.cat((h[0], h[1]), -1))
